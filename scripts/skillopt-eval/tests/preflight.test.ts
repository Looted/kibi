...
describe("SkillOpt host preflight", () => {
  test("passes only when every host, model, and isolation dependency is available", () => {
    ...
    expect(receipt.hosts).toHaveLength(3);
  ...
  test("live CLI reports a no-go without launching a model", () => {
    ...
    expect(process.stdout.toString()).toContain('"reason":"missing_host');
  });
});
