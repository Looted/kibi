...
function evidenceFrom(rawCalls: CellEvidence["broker"]["rawCalls"]): CellEvidence {
  return {
...
describe("migration apply contract verifier", () => {
  ...
  function coverageCall() {
    return {
      ...
      result: {
        structuredContent: {
          kibiProtocol: 1,
          data: { planHash: PLAN.planHash, plan: PLAN, actions: PLAN.actions },
        },
      },
    };
  }
  ...
  test("accepts the exact single-action application", () => {
    const violations = migrationApplyContractViolations(contract, evidenceFrom([
      coverageCall(),
      applyCall(),
    ]));
