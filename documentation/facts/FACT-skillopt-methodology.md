...
`--max-steps` is the number of complete proposal rounds and is constrained to 1–4. Each round rolls out all eight public training cases, reflects once with the structured public evidence, requests one optimizer rewrite, and scores that proposal on all four development cases. Retry once only for pre-action spawn/provider/transport/MCP handshake failures or a verified Codex crash. A refusal, malformed output, wrong/no tool use, or agent-caused evaluator miss is a terminal behavioral failure with any earned partial score retained; infrastructure, timeout, budget, evidence-conflict, and security outcomes remain zero and stop according to their typed policy.
