...
SkillOpt may propose at most four candidates per skill. Stop after two consecutive proposals improve the worst-family development score by less than one point. Retry once only for pre-action spawn/provider/transport/MCP handshake failures or a verified Codex crash. A timeout after generation, refusal, malformed output, wrong/no tool use, agent-caused evaluator failure, or budget termination is a terminal behavioral failure.
