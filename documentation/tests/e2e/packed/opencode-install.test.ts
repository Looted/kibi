...
if (RUN_NODE_TEST_SUITE) {
  describe("E2E: kibi-opencode tarball install and plugin invocation", {
    timeout: 300000,
  }, () => {
    let tmpDir: string;
    ...
    it("installs kibi-opencode package with correct version", {
      timeout: 30000,
    }, () => {
    ...
    it("dist/index.js is present after tarball install", {
      timeout: 30000,
    }, () => {
    ...
    it("plugin default export is callable and returns hooks without throwing", {
      timeout: 60000,
    }, async () => {
      const distIndex = join(
        installDir,
    ...
    it("subpath exports are accessible after install", {
      timeout: 30000,
    }, async () => {
      ...
      const fileFilterModule = await import(join(base, "dist/file-filter.js"));
