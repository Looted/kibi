...
function archivedTitle(index: number): string {
  const prefix = `Archived test metadata payload ${String(index).padStart(4, "0")} payload=`;
...
export async function runProofAllBoundedProjectionWorkflow(): Promise<{
  ...
  try {
    ...
    for (const index of Array.from(
      ...
      const target = join(sandbox.repoDir, ".kb/tests", `${document.id}.md`);
    ...
    const upsert = await kibi(sandbox, ["upsert", "--input", symbolUpsertPath]);
    ...
    assertCommandSucceeded(
      stagedSymbolArtifacts,
      "stage generated symbol artifacts",
    );
    ...
    assertCommandSucceeded(
      symbolCommit,
      "commit projection producer symbol evidence",
    );
    ...
    for (;;) {
      ...
      assertCommandSucceeded(
        page,
        `query full test properties at offset ${offset}`,
      );
      queryPageBytes.push(Buffer.byteLength(page.stdout, "utf8"));
      const pageByteSize = queryPageBytes.at(-1);
      assert.ok(
        pageByteSize !== undefined && pageByteSize < 8 * 1024 * 1024,
    ...
    assert.ok(
      ...
      `the exact combined full-property JSON payload must exceed 10 MiB; bytes=${reconstructedFullPropertyJsonBytes}`,
    );
    ...
    assert.equal(Array.isArray(recorded?.proof_receipts), true);
    ...
    assert.match(String(receipts?.[0]?.code_snapshot ?? ""), /^[a-f0-9]{64}$/);
