...
describe('Packed E2E: CLI Query regression (consumer issue #3)', () => {
  ...
  it('should return entities when KB has data', { timeout: 60000 }, async () => {
    ...
    // Sync to populate .kb
    const { exitCode: syncCode, stdout: syncOut } = await kibi(sandbox, ['sync']);
    assert.strictEqual(syncCode, 0, `kibi sync should succeed: ${syncOut}`);

    // Query for req and assert REQ-001 is present in stdout
    const { exitCode: qCode, stdout } = await kibi(sandbox, ['query', 'req']);
    assert.strictEqual(qCode, 0, 'kibi query should exit 0');
    assert.ok(!stdout.includes('No entities found'), 'query should not claim no entities found');
    assert.ok(stdout.includes('REQ-001') || stdout.includes('E2E requirement'), 'output must include created requirement');
  });

  it('should return empty when KB has no matching entities', { timeout: 60000 }, async () => {
    ...
    const { exitCode: qCode, stdout } = await kibi(sandbox, ['query', 'flag', '--format', 'json']);
    assert.strictEqual(qCode, 0, 'kibi query should exit 0 even when empty');
    // For JSON format empty result should be []
    assert.ok(stdout.trim() === '[]' || stdout.includes('No entities found'), 'Expected explicit empty output for no entities');
  });

  it('should distinguish legitimately empty from falsely empty and work from different CWD', { timeout: 60000 }, async () => {
    ...
    const reqRes = await kibi(sandbox, ['query', 'req', '--format', 'table']);
    assert.strictEqual(reqRes.exitCode, 0);
    assert.ok(!reqRes.stdout.includes('No entities found'), 'req query should return entities');
    assert.ok(reqRes.stdout.includes('REQ-001') || reqRes.stdout.includes('E2E requirement'));

    // Query a different type that we expect to be empty (e.g., symbol)
    const symRes = await kibi(sandbox, ['query', 'symbol', '--format', 'json']);
    assert.strictEqual(symRes.exitCode, 0);
    // JSON empty should be []
    assert.ok(symRes.stdout.trim() === '[]' || symRes.stdout.includes('No entities found'));
    ...
    const { stdout: outsideStdout, exitCode: outsideCode } = await run('node', [sandbox.kibiBin, 'query', 'req', '--format', 'table'], {
      cwd: sandbox.baseDir,
      env: sandbox.env,
      timeoutMs: 60000,
    });
    assert.strictEqual(outsideCode, 0, 'external invocation should exit 0');
    assert.ok(!outsideStdout.includes('No entities found'), 'external invocation must find entities');
    assert.ok(outsideStdout.includes('REQ-001') || outsideStdout.includes('E2E requirement'));
  });
});
