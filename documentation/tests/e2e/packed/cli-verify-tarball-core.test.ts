...
describe('Packed E2E: verify tarball contents (core present)', () => {
  ...
  it('kibi-core tarball must contain src/kb.pl and .pl runtime files', () => {
    ...
    assert.ok(tgz && tgz.endsWith('.tgz'), `Expected core tarball path, got: ${tgz}`);
    ...
    const hasPlFiles = entries.some(e => e.startsWith('src/') && e.endsWith('.pl'));

    for (const r of required) {
      if (!entries.includes(r)) {
        throw new Error(`Missing required file '${r}' in kibi-core tarball (${tgz})`);
      }
    }

    if (!hasPlFiles) {
      throw new Error(`No Prolog (.pl) files found under src/ in kibi-core tarball (${tgz})`);
    }
  });

  it('kibi-cli tarball must contain bin/kibi, dist/cli.js and declare dependency on kibi-core', () => {
    const tgz = tarballs.cli;
    assert.ok(tgz && tgz.endsWith('.tgz'), `Expected cli tarball path, got: ${tgz}`);
    ...
    for (const r of required) {
      if (!entries.includes(r)) {
        throw new Error(`Missing required file '${r}' in kibi-cli tarball (${tgz})`);
      }
    }

    // Inspect package.json inside the tarball to ensure it lists kibi-core as dependency
    const pkgJsonRaw = execFileSync('tar', ['-xOzf', tgz, 'package/package.json'], { encoding: 'utf8' });
    let pkg: any;
    try {
      pkg = JSON.parse(pkgJsonRaw);
    } catch (e) {
      throw new Error(`Failed to parse package.json from kibi-cli tarball (${tgz}): ${(e as Error).message}`);
    }

    const deps = { ...(pkg.dependencies || {}), ...(pkg.peerDependencies || {}) };
    if (!deps['kibi-core']) {
      throw new Error(`kibi-cli package.json does not declare dependency on 'kibi-core' (found: ${Object.keys(deps).join(', ')})`);
    }
  });

  it('kibi-mcp tarball must contain bin/kibi-mcp and dist/server.js', () => {
    const tgz = tarballs.mcp;
    assert.ok(tgz && tgz.endsWith('.tgz'), `Expected mcp tarball path, got: ${tgz}`);
    ...
    for (const r of required) {
      if (!entries.includes(r)) {
        throw new Error(`Missing required file '${r}' in kibi-mcp tarball (${tgz})`);
      }
    }
  }, 30000);
