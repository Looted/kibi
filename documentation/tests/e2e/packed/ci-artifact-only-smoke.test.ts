...
const RUN_NODE_TEST_SUITE = typeof (globalThis as { Bun?: unknown }).Bun === "undefined";

if (RUN_NODE_TEST_SUITE) {
  describe("Packed CI: artifact-only smoke (no checkout)", { timeout: 180000 }, () => {
    ...
    before(async () => {
      ...
      await run("mkdir", ["-p", join(artifactRoot, "core")], { cwd: "/tmp", env: process.env });
      await run("mkdir", ["-p", join(artifactRoot, "cli")], { cwd: "/tmp", env: process.env });
      await run("mkdir", ["-p", join(artifactRoot, "mcp")], { cwd: "/tmp", env: process.env });

      await run("cp", [tarballs.core, join(artifactRoot, "core")], { cwd: "/tmp", env: process.env });
      await run("cp", [tarballs.cli, join(artifactRoot, "cli")], { cwd: "/tmp", env: process.env });
      await run("cp", [tarballs.mcp, join(artifactRoot, "mcp")], { cwd: "/tmp", env: process.env });
    ...
    it("happy-path group A: cli workflows + query + verify-core", async () => {
      ...
      // kibi --version
      const { exitCode: vCode } = await run("node", [sandbox.kibiBin, "--version"], { cwd: sandbox.repoDir, env: sandbox.env });
      assert.strictEqual(vCode, 0, "kibi --version should succeed");

      // init
      const { exitCode: initCode } = await run("node", [sandbox.kibiBin, "init", "--no-hooks"], { cwd: sandbox.repoDir, env: sandbox.env, timeoutMs: 120000 });
      assert.strictEqual(initCode, 0, "kibi init should succeed");

      // sync (should succeed even in minimal workspace)
      const { exitCode: syncCode } = await run("node", [sandbox.kibiBin, "sync"], { cwd: sandbox.repoDir, env: sandbox.env, timeoutMs: 120000 });
    ...
    it("happy-path group B: mcp + branch-workflow basics", async () => {
      ...
      const { exitCode: execCode } = await run("test", ["-x", sandbox.kibiMcpBin], { cwd: sandbox.repoDir, env: sandbox.env });
      assert.strictEqual(execCode, 0, `kibi-mcp binary should exist and be executable at ${sandbox.kibiMcpBin}`);
    });
    ...
    it("failure-path: missing tarballs reported", async () => {
      ...
      await run("mkdir", ["-p", emptyRoot], { cwd: "/tmp", env: process.env });
      ...
      const { exitCode, stdout, stderr } = await run("node", ["--input-type=module", "-e", expr], { cwd: "/tmp/kibi-e2e-packed-compiled", env: process.env, timeoutMs: 120000 });

      // Expect non-zero exit and error about missing pre-packed tarball
      assert.notStrictEqual(exitCode, 0, `Expected packAll() in fresh process to fail, stdout: ${stdout}, stderr: ${stderr}`);
      const combined = `${stdout}\n${stderr}`;
      assert.ok(combined.includes("Pre-packed tarball not found for package:"), `Expected missing tarball error, got: ${combined}`);
    });
  });
}
