  ...
  describe("E2E: Discovery bundle parity", () => {
    ...
    before(
      async () => {
        ...
        if (!hasProlog) {
          console.warn("⚠️  SWI-Prolog not available, skipping discovery bundle E2E");
          return;
        }
        ...
        fs.writeFileSync(
          join(sandbox.repoDir, "documentation", "requirements", "REQ-DISC-001.md"),
          `---
id: REQ-DISC-001
...
`,
        ...
        fs.writeFileSync(
          join(sandbox.repoDir, "documentation", "scenarios", "SCEN-DISC-001.md"),
          `---
id: SCEN-DISC-001
...
`,
        ...
        fs.writeFileSync(
          join(sandbox.repoDir, "documentation", "requirements", "REQ-DISC-002.md"),
          `---
id: REQ-DISC-002
...
`,
    ...
    it("keeps CLI and MCP aligned for search/status/gaps/coverage/graph", { timeout: 120000 }, async () => {
      if (!hasProlog) return;
      ...
      const coverageCliJson = JSON.parse(coverageCli.stdout) as {
        summary: { fullyCovered: number; notApplicable: number; total: number };
        rows: Array<{ id: string; coverageStatus: string; evaluated: boolean }>;
      };
      ...
      assert.ok(graphCliJson.nodes.some((node) => node.id === "REQ-DISC-001"));
      assert.ok(graphCliJson.nodes.some((node) => node.id === "SCEN-DISC-001"));
      ...
      try {
        ...
        const searchStructured = (searchMcp.result?.structuredContent ?? {}) as {
          count?: number;
          results?: Array<{ entity: { id: string } }>;
        };
        ...
        const statusStructured = (statusMcp.result?.structuredContent ?? {}) as {
          branch?: string;
          dirty?: boolean;
        };
        ...
        const gapsMcp = await sendMcpRequest(mcp, {
          ...
          params: {
            name: "kb_find_gaps",
            arguments: { type: "req", missingRelationships: ["specified_by"] },
          },
        });
        ...
        assert.strictEqual(gapsStructured.rows?.[0]?.id, gapsCliJson.rows[0]?.id);
        ...
        const coverageStructured = (coverageMcp.result?.structuredContent ?? {}) as {
          summary?: { fullyCovered?: number; notApplicable?: number; total?: number };
          rows?: Array<{ id: string; coverageStatus: string; evaluated: boolean }>;
        };
        assert.strictEqual(
        ...
        assert.strictEqual(coverageStructured.summary?.total, coverageCliJson.summary.total);
        assert.ok(
        ...
        const graphStructured = (graphMcp.result?.structuredContent ?? {}) as {
          nodes?: Array<{ id: string }>;
        };
        assert.ok(graphStructured.nodes?.some((node) => node.id === "REQ-DISC-001"));
        assert.ok(graphStructured.nodes?.some((node) => node.id === "SCEN-DISC-001"));
      } finally {
        await stopProcess(mcp);
      }
    });

    it("writes derived diagnostic fields in MCP diagnostic mode", { timeout: 120000 }, async () => {
      if (!hasProlog) return;
      ...
      try {
        ...
        const logLines = fs.readFileSync(usageLogPath, "utf8").trim().split("\n");
