...
5. Provide dynamic, contextual prompt guidance based on recent edits and workspace state, including targeted nudges for:
   ...
   - ADR chain awareness
6. Emit loud warnings when agents attempt manual edits under `.kb/**`, directing them toward public MCP/CLI tools.
7. Detect and warn on invalid Kibi authoring patterns, such as embedded scenarios/tests in requirements.
8. Run targeted validation checks after relevant KB-document edits (e.g., `kibi check --rules must-priority-coverage,no-dangling-refs`).
9. Detect uninitialized or weakly bootstrapped repos and nudge toward `/init-kibi`, `kibi init`, or `kibi doctor`.
