...
7. For sanctioned start-task briefing guidance, the plugin may suggest `/brief-kibi`, which maps to `kb_briefing_generate` on the MCP surface.
8. Relevant edits still trigger background sync and validation automatically.

### Expected Outcomes

- No agent-visible guidance tells the agent to run direct `kibi` CLI commands.
- The agent can complete KB work with MCP tools and sanctioned slash commands (`/init-kibi`, `/brief-kibi`).
