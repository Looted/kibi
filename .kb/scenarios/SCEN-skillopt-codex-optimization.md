...
Given provider output reporting exhausted credits or a usage limit, when episode replay classifies the terminal state, then it emits `budget-exhausted` even if normal broker, diagnostic, or final-state artifacts were not written. Given an ordinary behavioral miss without a critical assertion, the bridge supplies a stable feedback category rather than an unknown failure pattern.

Given a safe-mutation task, when its fixture and independent final state are evaluated, then the public request names the exact production symbol, requirement, test, and typed relationships, qualifying test evidence is present, and completion requires both ownership and coverage edges rather than any clean query/check pair.
