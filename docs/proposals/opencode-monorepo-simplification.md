...
This document outlines the recommendations for simplifying the Kibi monorepo, distinguishing between completed work and future improvements.

## Completed in this change

The following items have been implemented as part of the initial extraction workstream:

### Startup Seam Extraction
- **Description**: Extracted the startup notification logic from the main `kibi-opencode` entry point into a dedicated `plugin-startup.ts` adapter.
- **Details**:
    - Added `ux.toastStartup` configuration to control startup notifications.
    - Implemented `startup-notifier.ts` as a clean adapter for notification logic.
    - Simplified `packages/opencode/src/index.ts` by removing direct notification side effects.
- **Payoff**: High. Improves testability and allows users to opt-out of "noisy" startup toasts.
- **Risk**: Low. Minimal impact on core logic.
- **Constraint/ADR**: ADR-012 (Internal Plugin Lifecycle).

## Package Boundary Assessment
Current package boundaries (`core`, `cli`, `mcp`, `opencode`, `vscode`) are sound and provide appropriate isolation for their respective runtimes and responsibilities.
...
- **Description**: Create a `packages/types` or `packages/shared` to hold common interfaces used across `core`, `cli`, and `mcp`.
- **Payoff**: Reduces duplication and prevents circular dependency risks when adding new protocols.
- **Risk**: Medium. Requires updating imports across the entire monorepo.
- **Constraint/ADR**: ADR-014 (Monorepo Shared Types).

## Deferred

### CLI/MCP Consolidation
- **Description**: Evaluate merging `packages/cli` and `packages/mcp` into a single `packages/host` if the overlap continues to grow.
- **Payoff**: Simplifies dependency management and release coordination.
- **Risk**: High. Could lead to a "god package" if not carefully bounded.
- **Constraint/ADR**: Deferred until v1.0 stability.

## Not Recommended

### Merging Core and CLI
- **Description**: Combining the Prolog core with the Node.js CLI.
- **Payoff**: Low. The runtime environments (Prolog vs Node.js) are fundamentally different.
- **Risk**: Very High. Breaks the clear separation between the knowledge engine and its consumers.
- **Constraint/ADR**: ADR-001 (Prolog Core Isolation).

---

## Future follow-up
- Standardize build artifacts across all packages using a shared `tsup` or `bun` build config.
- Implement automated version syncing for internal workspace dependencies.
