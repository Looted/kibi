...
- **2026-08-26 · Wave 1–2 · F-031 (blocker, resolved operationally) · Shared-worktree collision** — A second agent session committed feature work directly onto this sprint's branch and later merged everything into local `develop` while proof runs were in flight. Consequences observed: (a) verify ingest rejects receipts with `captured snapshot is not the live workspace snapshot` when ANY tracked file changes between snapshot capture and ingest — long (~15 min) contracts are maximally exposed; (b) two chained proof runs failed at different contracts with no product-level error; (c) `git reset --hard` silently failed once immediately after the kibi post-checkout hook ran (transient lock?). Resolution: paused the other session, reset sprint branch to local `develop`, re-synced. Suggestion: document "exclusive worktree required for contracted proof runs"; consider a runner preflight that aborts early if `git status` is not clean instead of dying 20 minutes later.
- **2026-08-26 · Wave 1–2 · F-032 (friction, CI)** — PR #220's proof job failed with **zero log output after the sync step** (21 minutes of silence, then failure): consistent with a GitHub runner crash/OOM during the heavyweight `TEST-test-journaled-engine-harness` contract (host-repo root suite, 4300+ tests). `gh run view --log-failed` shows nothing actionable. Suggestion: give the proof job an explicit `timeout-minutes`, split the harness contract into its own job, or stream per-contract progress markers to a step summary so crashes are attributable.
- **2026-08-26 · Wave 1–2 · F-033 (data quality, fixed in-sprint)** — The `chore(release): version packages` commit bumped `kibi-codex`/`kibi-cursor` peerDependencies from `>=0.12.0 <1` to `>=1.0.0` but left both `package-contract.test.ts` suites asserting the old ranges → red unit suite at local `develop` tip. Note also that origin `develop` tip (`4cf383b6`) has **never been CI-green**: develop's last successful CI ran at `5af1fc64`. Fixed here by updating the two tests to the released contract. Suggestion: version-packages flow should grep for pinned ranges in contract tests, or the contract should read expected ranges from a shared source of truth.
- **2026-08-26 · Wave 1–2 · observation** — After merging the other agent's work, missing-semantic-inventory gaps dropped 41 → 38 without sprint action (their commits grounded several requirements), and sync runtime improved (3m22s → ~35–59s incremental).
- **2026-08-26 · Wave 2 · F-034 (blocker, product bug) · Host file-edit hook destroys proof freshness during active sessions** — The opencode host's kibi plugin auto-syncs the real worktree on **every tracked-file edit**. Any write — including `proof/baseline.json` itself, minutes after a clean 29-contract run — triggers a sync that rewrites `.kb/symbols.yaml` and flips coverage from 35/97 proven to 0/97 (`stale_verification_receipt` 3→44). Combined with F-031 this explains every "phantom" ingest failure and coverage collapse observed across waves: long proof runs cannot coexist with an attached editing session. Mitigation used: kill the session's kibi-mcp/server, make no edits while the runner is live, and accept that post-run measurements must be taken before any further writes. Suggestion: the hook should debounce/no-op when a sync already ran for the current tree state, and the runner should hold an explicit worktree lock that the hook respects.
- **2026-08-26 · Wave 2 · F-035 (data quality) · Proof-state wobble across sync cycles** — At one tree, consecutive measurements gave 40/97 proven (mid-churn), then 0 (hook artifact), then 35/97 twice (settled, reproducible). `missing_production_symbol_coverage` wobbled 45↔50 and `incomplete_semantic_inventory` 0↔2 for the same content depending on when symbol refresh ran relative to receipts. Additionally, 3 receipts remain `stale_verification_receipt` after two full runner cycles — the runner ingests them as passed yet coverage still flags them. Suggestion: snapshot/hash derivation for receipts and coverage must be computed from the same canonical function, and the runner should reconcile the small persistent stale set instead of leaving a standing contradiction between "contract passed" and "receipt stale".
- **2026-08-26 · Wave 2 · F-036 (blocker, fixed in-sprint) · Append-only receipt history had a terminal cap** — PR #221's proof job passed every selected contract but failed ingesting the last result because persisted `verification_receipts` was both append-only and entity-schema-capped at 50 items. Once a test reached 50 historical receipts, verification ingest could not append fresh proof. Fixed by removing the contradictory persisted-entity cap while preserving append-only validation; added schema coverage for 51 receipts.
- **2026-08-26 · Wave 2 · ratchet update** — The first complete PR #221 proof run after the receipt fix reached **39 / 97 current proven**, reduced `missing_passing_e2e` 56→53 and `missing_production_symbol_coverage` 50→46, and eliminated the tracked `stale_verification_receipt` gap. Baseline and contract expectations were raised explicitly.
