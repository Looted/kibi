...
The exact Pyright npm archive also includes `dist/typeshed-fallback/LICENSE` (12,657 bytes, SHA-256 `295f8538c94ae5c3043301cf7cff1c852dab6a786a8ddee471e061b40d5ecabe`). It contains Apache-2.0 terms for typeshed and an appended MIT section for separately licensed parts; Pyright's own MIT label does not describe every bundled component. Its optional `fsevents` dependency is Darwin-only and was absent in this Linux installation. These servers and their bundled assets are operator tools used by the spike, and are not redistributed in the parser package.
