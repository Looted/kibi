...
9. Require the four-case public development result to reach mean `0.85`, at least three hard passes, and worst-family mean `0.75`, while also improving mean without hard-pass or worst-family regression against the stronger comparator. A miss returns `development_gate_ineligible` with held-out `not-run`; a pass proceeds to the blinded held-out aggregate gates. This absolute admission floor prevents a weak relative improvement from spending the 96-cell sealed matrix. The real cells reuse one private staged Codex/bwrap runtime for the entire run. Each non-Git fixture pins the target, Codex MCP configuration, broker, and independent verifier to the same `skillopt-eval` Kibi branch; target-only MCP approval is limited to the evaluator-owned allowlisted broker.
