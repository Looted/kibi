...
mock.module("../src/codeActionProvider", () => ({
  ...
  browseLinkedEntities: (
    ...args: Parameters<typeof browseLinkedEntitiesMock>
...
function configureVscodeMock() {
  ...
  const openDocListeners: Array<
    (document: { uri: { fsPath: string; scheme: string } }) => unknown
  > = [];
  ...
  Object.assign(vscode.window as Record<string, unknown>, {
    ...
    showTextDocument: mock(async (_doc: unknown, _options?: unknown) => ({
      selection: undefined,
    })),
  ...
  Object.assign(vscode.languages as Record<string, unknown>, {
    ...
    registerCodeLensProvider: mock(
      (_selector: unknown, _provider: unknown) => ({ dispose() {} }),
    ),
...
describe("activation/navigation", () => {
  test("registers navigation commands and handles open/info/error flows", async () => {
    ...
    const showInfo = vscode.window.showInformationMessage as ReturnType<
      typeof mock
    >;
    ...
    const treeDataProvider = {
      ...
      getNavigationTargetForEntity: (entityId: string) => {
        if (entityId === "REQ-OK")
          return { localPath: "/docs/req.md", line: 7 };
        if (entityId === "REQ-FAIL")
          return { localPath: "/docs/fail.md", line: 3 };
        return undefined;
      },
    };

    navigationModule.registerNavigationCommands(
      output as never,
      treeDataProvider,
    );

    await getCommandCallback(commandCalls, "kibi.openEntity")(
      "/docs/file.md",
      2,
    );
    await getCommandCallback(commandCalls, "kibi.openEntity")(
    ...
    expect(output.appendLine).toHaveBeenCalledWith(
      "Navigation commands registered.",
    );
    ...
    expect(showError).toHaveBeenCalledWith(
      "Kibi: Could not open file — /docs/fail.md",
    );
    expect(showError).toHaveBeenCalledWith(
...
describe("activation/traceability", () => {
  test("registers traceability features and wires browse command navigation fallback", async () => {
    ...
    expect(output.appendLine).toHaveBeenCalledWith(
      "Traceability code actions initialized.",
    );
    expect(output.appendLine).toHaveBeenCalledWith(
    ...
    expect(navigationResolver?.("REQ-001")).toEqual({
      localPath: "/docs/req.md",
      line: 12,
    });
    expect(navigationResolver?.("REQ-FALLBACK")).toEqual({
      localPath: "/fallback/req.md",
    });
  });
  ...
  test("reports initialization failures without blocking remaining features", () => {
    const output = createOutput();
    const context = createContext();
    const showWarning = vscode.window.showWarningMessage as ReturnType<
      typeof mock
    >;
...
describe("activation/contextOnOpen", () => {
  test("skips registration when contextOnOpen is disabled", () => {
    ...
    contextOnOpenModule.registerContextOnOpen(
      context as never,
      output as never,
      tmpDir,
    );
  ...
  test("queries MCP on file open and logs failures", async () => {
    ...
    contextOnOpenModule._setContextOnOpenFsDepsForTests({
      existsSync: () => true,
    });
    contextOnOpenModule.registerContextOnOpen(
      context as never,
    ...
    await listener({
      uri: { scheme: "file", fsPath: path.join(tmpDir, "src", "main.ts") },
    });
    ...
    await listener({
      uri: { scheme: "untitled", fsPath: path.join(tmpDir, "src", "main.ts") },
    });
