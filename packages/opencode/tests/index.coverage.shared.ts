...
describe("index coverage", () => {
  ...
  test("returns empty hooks when the plugin is disabled", async () => {
    const hooks = await createHooks(plugin, tmpDir, [], { enabled: false });
...
`,
    ...
    const hooks = await createHooks(plugin, tmpDir, logs, {
      enabled: true,
      sync: { enabled: false },
      guidance: {
  ...
  test("ignores missing relative requirement files without throwing", async () => {
    const hooks = await createHooks(plugin, tmpDir, [], {
      enabled: true,
      sync: { enabled: false },
      guidance: {
        sessionSummary: { enabled: false },
...
`,
  ...
  test("invalidates guidance cache when branch and config fingerprints change", async () => {
    ...
    const initialHooks = await createHooks(plugin, tmpDir, [], {
      enabled: true,
      prompt: { enabled: true, hookMode: "auto" },
      guidance: {
        sessionSummary: { enabled: false },
    ...
    initGitRepo(tmpDir);
    await createHooks(plugin, tmpDir, [], {
      enabled: true,
      prompt: { enabled: true, hookMode: "auto" },
      guidance: {
        sessionSummary: { enabled: false },
