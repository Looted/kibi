...
function parseChangesetLine(line: string): { timestamp: string; operation: string; entityId: string } | null {
...
export function computeAuditDelta(
  ...
  if (!fs.existsSync(auditPath)) {
    return {
      hasChanges: false,
      entries: [],
      newCursor: previousCursor ?? { lastTimestamp: "", lastOperation: "", entryCount: 0, fileSize: 0 },
      contentHash: "",
    };
  }
  ...
  if (previousCursor && fileSize === previousCursor.fileSize && lines.length === previousCursor.entryCount) {
    return {
      hasChanges: false,
      entries: [],
  ...
  if (previousCursor && previousCursor.lastTimestamp) {
    const lastIdx = entries.findIndex(
      (e) => e.timestamp === previousCursor.lastTimestamp && e.operation === previousCursor.lastOperation
    );
    if (lastIdx >= 0) {
...
export function getLatestAuditCursor(
  ...
  for (const file of files) {
    try {
      const brief: IdleBriefEnvelope = JSON.parse(fs.readFileSync(file.path, "utf-8"));
