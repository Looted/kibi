...
| `kb_status` | `status` | `--input JSON or flags` | read | no | `kb-read, workspace-read` | peer; capability-selected |
...
| `kb_apply_plan` | `apply-plan` | `--input JSON` | write | no | `kb-read, kb-write, workspace-read, workspace-write` | peer; capability-selected |
