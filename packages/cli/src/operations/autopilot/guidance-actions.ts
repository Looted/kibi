...
export function buildActions(
  ...
  declared: AutopilotDeclaredContext,
  ...
  if (activation.applyBlocked)
    ...
    result.push({
      order: order++,
      kind: "upsert",
      description: `Review and optionally upsert ${candidates.length} safe candidate(s) from deterministic evidence.`,
  ...
  result.push({
    ...
    description:
      declared.verificationAnchors.length > 0
        ? `After approved kb_upsert calls, run kb_check and verify ${summary(declared.verificationAnchors, 2)}.`
        : "After approved kb_upsert calls, run kb_check to validate the resulting graph.",
  });
  return result;
}
