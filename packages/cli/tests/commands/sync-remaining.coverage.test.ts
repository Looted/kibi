...
describe("syncCommand remaining runtime branches", () => {
  ...
  test("recoveryBackupPath recovers missing pending relationship receipts", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    git(cwd, "add .kb");
  ...
  test("fast-path and extra-relationship-hash no-op checkpoints fail closed", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
  ...
  test("relationship-only shard edits use the journaled entity-delta path", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
  ...
  test("unreadable relationship shards warn and stale cache hashes invalidate the fast path", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
  ...
  test("v1 cache relationship changes retry then fail closed on retract-all", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
  ...
  test("validate-only mixed authoring errors and dangling shards take the staging failure path", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
  ...
  test("journaled write records docs-not-indexed when a full reindex drops files", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
  ...
  test("recovery publication disappears, collides, and rolls back a failed staging rename", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    git(cwd, "add .kb");
    git(cwd, "commit --no-verify -m init-kb");
  ...
  test("rebuild of a journaled store publishes a replacement generation", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    ...
    expect(first.success).toBe(true);
    expect(
      existsSync(path.join(branchStorePath(cwd, "main"), "storage.json")),
    ).toBe(true);
  ...
  test("staging write invalidates the Prolog cache when the compiled KB changed", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
  ...
  test("pending relationship receipts reject escapes, drift, and missing shards", async () => {
    ...
    writePendingSourceReceipt(cwd, ".kb/relationships/aa.yaml", "b".repeat(64));
    await expect(
      syncCommand(
        { validateOnly: true, workspaceRoot: cwd },
        { createProlog: () => scriptedProlog() as never },
  ...
  test("warm-cache coordinate refresh still requeues an unchanged manifest", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
...
`,
  ...
  test("relationship-only shard removal uses the journaled entity-delta retract path", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
  ...
  test("replacing an entity id in a markdown source removes the previous id", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
...
`,
  ...
  test("test-only embedded authoring errors still emit invalid-authoring diagnostics", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
