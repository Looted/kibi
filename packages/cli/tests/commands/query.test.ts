...
`,
  ...
  test("queries all entities of a type", () => {
  ...
  test("queries specific entity by ID", () => {
  ...
  test("queries relationships from entity", () => {
    const allOutput = execSync(`bun ${kibiBin} query req --format json`, {
      cwd: tmpDir,
      encoding: "utf8",
    });
    ...
    const allResults = JSON.parse(allOutput);
    const entityWithLinks = allResults.find((r: any) =>
  ...
  test("rejects invalid entity type", () => {
    try {
      ...
      expect(error.stderr.toString()).toContain("Invalid type");
    }
  });

  test("handles empty table output gracefully", () => {
    const output = execSync(
