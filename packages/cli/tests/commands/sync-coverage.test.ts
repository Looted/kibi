...
describe("syncCommand write, cache, and pending-receipt paths", () => {
  test("write sync then a second call takes the no-op cache checkpoint", async () => {
  ...
  test("KIBI_DEBUG logs branch and source counts on validate-only", async () => {
    const cwd = preparedGitWorkspace();
  ...
  test("refreshSymbolCoordinates on a write sync after init", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    git(cwd, "add .kb");
  ...
  test("rebuild with recoveryBackupPath publishes a recoverable backup", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    git(cwd, "add .kb");
    git(cwd, "commit --no-verify -m init-kb");
  ...
  test("rebuild throws when a stale engine socket file is not reachable", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
  ...
  test("untracked pending relationship shards are compiled on write sync", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    git(cwd, "add .kb");
    git(cwd, "commit --no-verify -m init-kb");
  ...
  test("planted cache with an invalid shard relationship key fails closed", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    git(cwd, "add .kb");
    git(cwd, "commit --no-verify -m init-kb");
  ...
  test("scripted Prolog validate-only attaches, then fails closed on attach or save", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    git(cwd, "add .kb");
    git(cwd, "commit --no-verify -m init-kb");
  ...
  test("scripted write sync persists a relationship shard and retracts a deleted source", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    ...
    writeSyncCache(path.join(store, "sync-cache.json"), {
      ...
      seenAt: {
        ".kb/requirements/gone.md": new Date(
    ...
    await expect(
      ...
      /proposition-complete ingestion failed|dangling|Imported/,
    );
  ...
  test("rebuild with createProlog hits staging save failure and recovery backup collision", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
    git(cwd, "add .kb");
    git(cwd, "commit --no-verify -m init-kb");
    ...
    await expect(
      ...
      /Failed to save staging KB|Failed to clear changed relationship shards|Error/,
    );
  }, 90_000);

  test("expired cache and v1 entity-hash backfill still compile through scripted Prolog", async () => {
    const cwd = preparedGitWorkspace();
    await withCwd(cwd, () => initCommand({}));
