  ...
  test("scores matching and rejecting prose for every named rule", () => {
    expect(
      scoreExactPredicates1(
        schema("guard"),
        "The flag must stay disabled until review.",
      ),
    ).toBe(0.98);
    expect(
      scoreExactPredicates1(schema("guard"), "unrelated prose"),
    ).toBeNull();
    ...
    expect(
      scoreExactPredicates1(schema("conditional_behavior"), "plain sentence."),
    ).toBe(0);
    ...
    expect(
      scoreExactPredicates1(
        schema("dependency_rule"),
        "Sync requires login before compile.",
      ),
    ).toBe(0.98);
    expect(
      scoreExactPredicates1(schema("dependency_rule"), "Sync happens later."),
    ).toBe(0);

    expect(
      scoreExactPredicates1(
        schema("ownership_rule"),
        "The requirement is owned by the author.",
      ),
    ).toBe(0.98);
    expect(
      scoreExactPredicates1(
        schema("ownership_rule"),
        "The requirement has an author.",
      ),
    ).toBe(0);
    ...
    expect(
      scoreExactPredicates1(
        schema("availability_sla"),
        "The API should stay up.",
      ),
    ).toBe(0);

    expect(
      scoreExactPredicates1(
        schema("notification_route"),
        "The bot must notify owners by email.",
      ),
    ).toBe(0.98);
    expect(
      scoreExactPredicates1(
        schema("notification_route"),
        "The bot talks to owners.",
      ),
    ).toBe(0);
    ...
    expect(
      scoreExactPredicates1(
        schema("data_residency_rule"),
        "Customer records are encrypted.",
      ),
    ).toBe(0);
    ...
    expect(
      scoreExactPredicates1(schema("consent_rule"), "The form collects email."),
    ).toBe(0);
    ...
    expect(
      scoreExactPredicates1(
        schema("consistency_rule"),
        "Each scenario is named.",
      ),
    ).toBe(0);

    expect(
      scoreExactPredicates1(schema("unknown_rule"), "anything"),
    ).toBeNull();
  });
});
...
describe("scoreExactPredicates2 exact lanes", () => {
  test("scores matching, keyword-rejecting, and rejecting prose", () => {
    ...
    expect(
      scoreExactPredicates2(
        schema("migration_boundary_rule"),
        "Legacy YAML is deleted.",
      ),
    ).toBe(0);

    expect(
      scoreExactPredicates2(
        schema("absence_requirement"),
    ...
    expect(
      scoreExactPredicates2(
        schema("documentation_standard_rule"),
        "Public tools have names.",
      ),
    ).toBe(0);

    expect(
      scoreExactPredicates2(
        schema("warmup_policy_rule"),
    ...
    expect(
      scoreExactPredicates2(
        schema("warmup_policy_rule"),
        "The daemon starts quickly.",
      ),
    ).toBe(0);

    expect(
      scoreExactPredicates2(schema("unknown_rule"), "anything"),
    ).toBeNull();
  });
});
