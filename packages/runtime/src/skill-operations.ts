...
import type { OperationResult, OperationSpec } from "kibi-cli/operations";
import type { OperationContext } from "kibi-cli/operations/runtime-types";
import type { SkillManifest } from "./skill-system/types.js";
import {
  listBundledSkills,
  loadBundledSkill,
...
async function executeSkillsRead(
  ...
  return {
    content: [
      { type: "text", text: `Read bundled skill resource ${id}/${resource}` },
    ],
    structuredContent: {
      content: readBundledSkillResource(id, resource),
    },
  };
}
