#!/usr/bin/env node
/**
 * Kibi Codex plugin MCP launcher.
 *
 * Codex launches plugin MCP servers from the active session workspace and,
 * in the legacy `.codex-plugin` format, provides no plugin-root environment
 * or placeholder expansion for `.mcp.json`. The built `.mcp.json` therefore
 * inlines this script as `node -e <source>` (see scripts/build-mcp-config.mjs),
 * so the launcher always starts, resolves the workspace from its cwd, and:
 *
 ...
 *   like the previous `npx --no-install kibi-mcp` config and proxies stdio;
...
const SERVER_NAME = "kibi-codex-launcher";
...
const MISSING_KIBI_MCP_INSTRUCTIONS =
  ...
  "then restart the Codex session. No KB tools are exposed in this session.";
...
/**
 * Mirror of packages/codex/src/workspace-optin.ts. A workspace is opted in
...
/**
 * Probe the project-local kibi-mcp exactly like the previous MCP config:
 ...
 * mirrors the child's exit state. Codex signals the whole process group, so
 * the child receives shutdown signals directly; the explicit forwarding below
 * covers launchers that detach the child from the group.
 */
function proxyKibiMcp(options = {}) {
  ...
  return new Promise((resolveExit) => {
    ...
    try {
      ...
      stderr.write(
        `[kibi-codex] Failed to start kibi-mcp: ${error?.message ?? String(error)}\n`,
      );
      resolveExit(1);
      return;
    }
...
// Kept on one line: the inline `node -e` copy appends its own `main();`, and
// line coverage then only owes the gate expression, not the direct-CLI call.
