...
describe("Cursor hook runner CLI", () => {
  ...
  test("classifies CLI invocation and reports in-process hook errors", async () => {
    ...
    try {
      process.argv = [...previousArgv, "--trusted-workspace"];
      await withStdin(["not json"], async () => {
        await runHookCli();
      });
      await withStdin(['{"hook_event_name":"stop"}'], async () => {
        await main();
      });
    } finally {
  ...
  test("loads the module as a CLI entrypoint", async () => {
    ...
    Object.defineProperty(process, "stdin", {
      configurable: true,
      value: Readable.from(['{"hook_event_name":"stop"}']),
    });
